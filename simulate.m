...
for param_ix = 1:size(params,1)
    ...
    models_par{param_ix} = ar_train(A_train,B_train,Z_train,lambda, rsL2, spectrumA, spectrumB);
    new_pred5 = ar_predict(dn, pn, y_sim, models_par{param_ix}, 5);
    
    cw(param_ix) = corr(w(:), new_pred5.w(:));
    cy(param_ix) = mean(diag(corr(y_sim, new_pred5.rec)));
...
pred = ar_predict(dn, pn, y_sim, model, 5);
